...
with app.app_context():
    Base.metadata.drop_all(db.engine, [TvEpisodes.__table__, TvSeasons.__table__, LoggedContent.__table__, Users.__table__, Content.__table__, Books.__table__])
    db.create_all()
    ...
    # db.session.commit()
    s1e1 = TvEpisodes(title="Uno", descr="James McGill feels stuck as a public defendant, but sometimes, things change...", episode_num=1, minutes_len=53, season=seasonOne, show=show)
    s1e2 = TvEpisodes(title="Mijo", descr="Jimmy has to talk his way out of a difficult situation.", episode_num=2, minutes_len=46, season=seasonOne, show=show)
    # seasonOne.episodes.append(s1e1)
    # seasonOne.episodes.append(s1e2)
    s2e1 = TvEpisodes(title="Switch", descr="Jimmy gets a new job, while Mike deals with his new job", episode_num=1, minutes_len=47, season=seasonTwo, show=show)
    s2e2 = TvEpisodes(title="Cobbler", descr="Jimmy has to get creative to get Mike's business associate out of trouble.", episode_num=2, minutes_len=47, season=seasonTwo, show=show)
    ...
    # loggedMovie = LoggedContent(user=testuser, content_ref=s1e1, status='watched', rating=8.0, user_review='A good start to a GREAT show.')
